...
const SingleAccessCache = require('../singleaccesscache');
...
describe('Testing singleton -credentials client', async function (){
        ...
        it('Should get a which expires in 3 seconds',async function(){
            ...
            this.timeout(5000)
            let now = new Date();
            let p = await theCreds.getToken(now.getTime()+3000);
            assert.equal("0",p);
            let p2 = await theCreds.getToken();
            //Should be same token
            assert.equal(p,p2);
            //Wait three seconds should get new token;
            await awaitFor(4000);
            p2 = await theCreds.getToken();
        ...
        it('Should return in order',async function(){
            ...
            let tokens = await Promise.all([
                theCreds.getToken(1,1000),
                theCreds.getToken(now.getTime()+3000)
            ]);
        ...
        it('Should be possible to clear a token',async function(){
            ...
            const theCreds= new SingleAccessCache({
                 ...
                 let now = new Date();
            let token1 = theCreds.getToken(now.getTime()+3000);
            await theCreds.clearToken();
            let token2 = theCreds.getToken(now.getTime()+3000);
